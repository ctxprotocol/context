...
const TOOLS = [
  ...
  },
  {
    name: "get_oracles",
    description:
      "Get available gas price oracles for a blockchain network.",
    inputSchema: {
      type: "object" as const,
      properties: {
        chainId: {
        ...
        oracles: {
          ...
          items: {
            ...
            properties: {
              ...
              label: {
                type: "string",
                description: "Human-readable oracle label",
              },
              system: {
                type: "string",
        ...
        content: [
          {
            type: "text",
            text: JSON.stringify({
              error: "BLOCKNATIVE_API_KEY is not configured",
            }),
          },
        ],
        isError: true,
    ...
    try {
      switch (name) {
        case "get_gas_price": {
          ...
          // Flat, predictable structure for AI agents
...
app.listen(port, () => {
  console.log(`Blocknative MCP server listening on http://localhost:${port}`);
...
// Flat output types for structuredContent (MCP 2025-06-18)
...
// Clean parsing functions that return flat structures
